             ...
             XI += 4)
        {
            ...
            //if (_mm_movemask_epi8(WriteMask))
            {
                     ...
                     ++I)
                {
                    ...
                    Assert((FetchX >= 0.0f) && (FetchX < TextureWidth));
                    Assert((FetchY >= 0.0f) && (FetchY < TextureHeight));

                    //bilinear sample
                    uint8 *TexelPtr = (((uint8 *)TextureMemory) + FetchX * BITMAP_BYTES_PER_PIXEL + FetchY * TexturePitch);
